...
# 이렇게 하면 모델 출력을 더 직접적으로 해석할 수 있지만 이 접근법은 소프트맥스 출력을 사용할 경우 모든 모델에 대해 
# 정확하고 수치적으로 안정적인 손실 계산을 제공하는 것이 불가능하므로 권장하지 않습니다.
#? 로짓? 확률을 나타내는 값의 범위가 -무한대에서 무한대까지인 값
#? 활성화 함수? 신경망의 출력을 결정하는 함수
#? 활성화 함수로 베이킹? 활성화 함수를 마지막 레이어에 적용하는 것
#? tf.nn? TensorFlow의 모든 연산을 제공하는 모듈
